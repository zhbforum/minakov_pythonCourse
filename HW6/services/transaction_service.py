...
def transfer_money_logic(sender_account_id: int, receiver_account_id: int, amount: float, currency: str, cursor) -> dict:
    ...
    if sender_account_id == receiver_account_id:
        return api_response(False, "Cannot transfer money to the same account.", log_type=LOG_WARN)

    if amount <= 0:
        return api_response(False, "Transfer amount must be greater than zero.", log_type=LOG_WARN)
    ...
    try:
        ...
        logger.warning("Currency conversion failed: %s", str(e))
        return api_response(False, "Currency conversion error.", log_type=LOG_WARN)
...
def transfer_money(sender_account_id: int, receiver_account_id: int, amount: float, currency: str, cursor=None) -> dict:
    return transfer_money_logic(sender_account_id, receiver_account_id, amount, currency, cursor)

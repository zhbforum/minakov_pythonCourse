...
def test_successful_transfer(mock_cursor):
    sender = make_account(1, 100.0, "USD", 10)
    receiver = make_account(2, 50.0, "USD", 20)

    with patch("services.transaction_service.get_account_by_id") as mock_get_account, \
         patch("services.transaction_service.get_exchange_rate") as mock_get_rate, \
         patch("services.transaction_service.get_bank_name_by_id") as mock_get_bank_name:
...
def test_transfer_insufficient_funds(mock_cursor):
...
def test_transfer_account_not_found(mock_cursor):
    with patch("services.transaction_service.get_account_by_id") as mock_get_account:
...
def test_transfer_with_conversion(mock_cursor):
    sender = make_account(1, 100.0, "USD", 10)
...
def test_transfer_to_self(mock_cursor):
...
def test_transfer_zero_amount(mock_cursor):
    sender = make_account(1, 100.0, "USD", 10)
    receiver = make_account(2, 50.0, "USD", 20)
...
def test_exchange_rate_fetch_fail(mock_cursor):
    sender = make_account(1, 100.0, "USD", 10)
    receiver = make_account(2, 50.0, "EUR", 20)
